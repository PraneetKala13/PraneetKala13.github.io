      ...
      <div className="container mx-auto px-4 py-24">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-12">
            <Button
              variant="ghost"
              className="mb-6 text-muted-foreground hover:text-foreground hover:bg-muted -ml-2"
              onClick={() => navigate('/')}
            >
          ...
          {/* Featured Projects */}
          <div className="mb-16">
            <h2 className="text-2xl font-semibold mb-8 text-foreground">Featured Work</h2>
                ...
                return (
                  ...
                  >
                    ...
                    <CardContent className="p-6">
                      ...
                      <p className="text-sm text-[#A68868] mb-3 font-medium">{project.category} • {project.period}</p>
                      <p className="text-muted-foreground mb-4 text-sm leading-relaxed">{project.description}</p>
                      <div className="flex flex-wrap gap-2">
                        {project.technologies.map((tech) => (
                          <span key={tech} className="px-2 py-1 bg-[#4B6382] text-white rounded text-xs font-medium">
                            {tech}
                          </span>
                        ))}
                      </div>
          ...
          {/* Additional Projects */}
          <div className="mb-16">
            <h2 className="text-2xl font-semibold mb-8 text-foreground">Additional Projects</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {otherProjects.map((project) => {
                ...
                return (
                  <Card
                    key={project.title}
                    className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-border bg-white"
                  >
                    ...
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {project.liveUrl !== '#' && (
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 text-muted-foreground hover:text-foreground flex-shrink-0 ml-2"
                            onClick={() => window.open(project.liveUrl, '_blank')}
                          >
                      ...
                      <p className="text-xs text-[#A68868] mb-3 font-medium">{project.category} • {project.period}</p>
                      <p className="text-muted-foreground text-sm mb-4 leading-relaxed">{project.description}</p>
                      <div className="flex flex-wrap gap-1">
                        {project.technologies.map((tech) => (
                          <span key={tech} className="px-2 py-1 bg-[#4B6382] text-white rounded text-xs">
                            {tech}
                          </span>
                        ))}
                      </div>
          ...
          {/* Publication */}
          <div className="relative rounded-xl p-8 border border-border shadow-sm overflow-hidden mb-16">
            <div
            ...
            <div className="relative z-10">
              <div className="text-center mb-6">
                <h2 className="text-2xl font-semibold text-foreground mb-2">Research Publication</h2>
